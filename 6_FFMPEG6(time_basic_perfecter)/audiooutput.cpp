...
AudioOutput::AudioOutput(AVSync *avsync, AVRational time_base,const AudioPrams &audio_prams, AVFramQueue *frame_queue)
    :avsync_(avsync), time_base_(time_base),src_tgt(audio_prams),frame_queue(frame_queue)
{
                    ...
                    if(!is->swr_ctx_ || swr_init(is->swr_ctx_)<0){
                        ...
                        //没有读到解码后数据
                          ...
                          is->audio_buf_size=512;//填回0值
                           ...
                           len1=is->audio_buf1_size-is->audio_buf1_index;//剩余的长度
...
int AudioOutput::Init()
{
    ...
    dst_tgt.channels = wanted_spec.channels; //spec.channels;
    dst_tgt.fmt = AV_SAMPLE_FMT_S16;
    dst_tgt.freq = wanted_spec.freq ;// spec.freq;
    dst_tgt.channel_layout = av_get_default_channel_layout(2);
    dst_tgt.frame_size = 1024;//src_tgt_.frame_size;
    SDL_PauseAudio(0);
    LogInfo("AudioOutput::Init() leave");
    return 0;
}
